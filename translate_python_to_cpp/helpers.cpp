...
vector< vector<float> > normalize(vector< vector <float> > grid) {
	...
	*/
	...
	// Determine the sum of all probabilities in the grid
	...
	// Normalize all grid probabilities so that the total is 1
	for (int row_num=0; row_num<grid.size(); row_num++) {
		for (int col_num=0; col_num<grid[0].size(); col_num++) {
			newRow.push_back(grid[row_num][col_num] / total);
		}
...
vector < vector <float> > blur(vector < vector < float> > grid, float blurring) {
	...
	*/
	...
	for (int row_num=0; row_num<grid.size(); row_num++) {
		for (int col_num=0; col_num<grid[0].size(); col_num++) {
			gridVal = grid[row_num][col_num];
			for (int dx=-1; dx<2; dx++) {
				for (int dy=-1; dy<2; dy++) {
					mult = window[dx+1][dy+1];
					new_i = (row_num + dy) % grid.size();
					new_j = (col_num + dx) % grid[0].size();
