...
vector< vector <float> > initialize_beliefs(vector< vector <char> > grid) {
  ...
  */
  ...
  // New vector with the same dimensions as grid and all with value belief_per_cell
  ...
  */
  ...
  for (int row_num=0; row_num<height; row_num++) {
    for (int col_num=0; col_num<width; col_num++) {
      new_i = (row_num + dy) % height;
      new_j = (col_num + dx) % width;
      while (new_i < 0) {
        new_i += height;  // Scale new_i without altering the number of spaces moved
      }
      while (new_j < 0) {
        new_j += width;   // Scale new_j without altering the number of spaces moved
      }
      newGrid[new_i][new_j] = beliefs[row_num][col_num];
    }
  }
	...
	float p_miss)
{
  ...
  */
	...
	for (int row_num=0; row_num<grid.size(); row_num++) {
    for (int col_num=0; col_num<grid[0].size(); col_num++) {
      if (color == grid[row_num][col_num]) {
        newGrid[row_num][col_num] *= p_hit;
      }
      else {
        newGrid[row_num][col_num] *= p_miss;
      }
    }
  }
